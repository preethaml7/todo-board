...
**single-user**, self-hosted app intended to be exposed to the internet through
a **Cloudflare Tunnel** with your own IP-allowlist/WAF in front. Security was a
primary design goal.
...
- **Assets:** the single user's credentials, session tokens, and task data (in
  SQLite).
- **Trusted:** the host running Docker, the Docker volume (`todo-data`), the
  `SESSION_SECRET`, and the Cloudflare edge/tunnel.
- **Untrusted:** the public internet, request bodies, headers, cookies, and any
...
- **Optional trusted-proxy gate:** with `TRUSTED_PROXY_SECRET` set, any request
  ...
  exempt) — so hitting the origin directly, bypassing Cloudflare, fails.
- Container runs as a **non-root** user, **read-only root filesystem**, **all
  capabilities dropped**, `no-new-privileges`, published to **loopback only**.
...
| Threat | Mitigation |
| --- | --- |
| Credential stuffing / brute force | argon2id + per-account lockout + per-IP throttle + Cloudflare in front |
...
| Direct-origin access (bypassing Cloudflare) | `TRUSTED_PROXY_SECRET` gate + loopback binding |
...
- **Put Cloudflare Access (Zero Trust) in front** for an identity layer *before*
  the app's own login — strongly recommended for internet exposure.
...
- **In-memory throttle** resets on restart and is per-instance — fine for a
  single-container single-user app; Cloudflare handles volumetric abuse.
