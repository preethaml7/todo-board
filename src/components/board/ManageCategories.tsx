      ...
      {CATEGORY_COLORS.map((c) => (
        <button
          ...
          style={{
            ...
            borderRadius: 6,
            boxShadow:
              value === c ? "0 0 0 2px var(--accent)" : "none",
          }}
        >
          <span
            className="chip-dot"
...
export default function ManageCategories({
  ...
  async function add() {
    ...
    if (!name) return;
    setError("");
    const res = await createCategoryAction({ name, color: newColor });
    if (!res.ok || !res.data) {
  ...
  async function saveEdit(id: number, name: string, color: string) {
    const res = await updateCategoryAction(id, { name: name.trim(), color });
    if (!res.ok || !res.data) {
  ...
  async function remove(id: number) {
    if (
      !window.confirm(
        "Delete this category? It will be removed from any tasks using it.",
      )
    )
      return;
    const res = await deleteCategoryAction(id);
    if (!res.ok || !res.data) {
      setError(res.error ?? "Could not delete category.");
      return;
    }
    sync(res.data);
  }
                ...
                <input
                  ...
                  onBlur={(e) => {
                    if (e.target.value.trim() && e.target.value !== c.name)
                      saveEdit(c.id, e.target.value, c.color);
                  }}
                />
                <ColorPicker
                  value={c.color}
                  onChange={(color) => saveEdit(c.id, c.name, color)}
                />
                <button
